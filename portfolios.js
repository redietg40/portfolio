  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.skills && Array.isArray(data.skills) && data.skills.length > 0) {
        ...
        // If API skills are empty in a category, use defaults for that category
        if (SKILLS.frontend.length === 0) {
          SKILLS.frontend = [...DEFAULT_SKILLS.frontend]
        }
        if (SKILLS.backend.length === 0) {
          SKILLS.backend = [...DEFAULT_SKILLS.backend]
        }
        if (SKILLS.tools.length === 0) {
          SKILLS.tools = [...DEFAULT_SKILLS.tools]
        }
      }
